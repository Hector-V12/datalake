import os, tarfile
from urllib3.response import HTTPResponse

from pyspark.sql.session import SparkSession
from pyspark.context import SparkContext
from pyspark.sql.functions import explode, col, from_utc_timestamp

from minio import Minio

HOME = os.path.expanduser('~')
AIRFLOW_ROOT_FOLDER = HOME + "/airflow"

MINIO_ACCESS_KEY = "9XdCihYxLXyakPKwKocC"
MINIO_SECRET_KEY = "T06fdHyWIUa517m8uRwNKGgNmy9HbA6yjjmL2Fca"

def format_consumption_data():
    client = Minio(
    "127.0.0.1:9000",
    access_key=MINIO_ACCESS_KEY,
    secret_key=MINIO_SECRET_KEY,
    secure=False
    )

    bucket_name = "mydatalake"

    json: str = None
    response: HTTPResponse = HTTPResponse()

    try:
        response = client.get_object(bucket_name, "data/raw/digital_iservices/digital_iservices_data.json")
        json = response.data.decode()
    finally:
        response.close()
        response.release_conn()

    sc = SparkContext('local')
    spark = SparkSession(sc).builder.getOrCreate()

    df = spark.read.json(sc.parallelize([json]))

    new_df = df.withColumn("production_type", explode(col("actual_generations_per_production_type")))
    final_df = new_df.withColumn("values", explode(col("production_type.values")))
    result_df = final_df.select(
        col("values.start_date").alias("date"),
        col("values.value").alias("value"),
        col("production_type.production_type").alias("production_type")
    )

    # Normalizing date field into UTC
    result_df = result_df.withColumn("date", from_utc_timestamp("date", "UTC"))

    result_df.show()

    file_location = "data/formatted/digital_iservices/digital_iservices_data.parquet"
    local_path = f"{AIRFLOW_ROOT_FOLDER}/{file_location}"
    result_df.write.mode("overwrite").parquet(local_path)

    # Saving parquet archive file to distributed filesytem (S3-like)
    archive_path = f"{local_path}.tgz"
    with tarfile.open(archive_path, "w:gz") as tar:
        tar.add(local_path)

    client.fput_object(bucket_name, file_location, archive_path)